...
1. **绝对服从宪法标准**：《Prompts/逐字稿生成标准V2.0.md》是本系统的最高纲领，教案和逐字稿的每一个微观细节（互动形式、评价词汇、思政植入、字数控制等）必须严格遵循该文档。
2. **现实工程与常识优先**：在构建教案与话术逻辑时，严禁盲目顺从教材字面排版顺序。必须引导虚拟学生根据真实世界中的网络工程实践（如：数据流向、故障排查标准步骤等）进行合理的逻辑推演。
3. **客体化与纯净排版**：逐字稿禁止出现 `[面带微笑]` 等动作标签，必须面向全班进行“客体化复述”（禁止用“你”，必须用“他/这位同学”）。
...
1. **阅读最高纲领**：读取 `Prompts/逐字稿生成标准V2.0.md`。
2. **阅读执行引擎**：读取 `Prompts/Generate_Script_Prompt_V9.md`。
3. **执行**：根据 Step 1 的教案骨架，输出字数精准、互动达标、去除废话动作的 10 分钟逐字稿，并在末尾附上《标准V2.0》中要求的 Checklist。
